...
import { getSougeiSchedule, updateSougeiSchedule, deleteSougeiSchedule } from '../../api/sougeiScheduleApi';
...
function SougeiScheduleEditPage() {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        const res = await getSougeiSchedule(Number(id));
        const s = res.data;
        setForm({
          userName: s.userName,
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateSougeiSchedule(Number(id), form);
      toast.success('送迎スケジュールを更新しました');
      navigate('/sougei-schedule');
    } catch {
      toast.error('送迎スケジュールの更新に失敗しました');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!window.confirm('この送迎スケジュールを削除しますか？')) return;
    setDeleting(true);
    try {
      await deleteSougeiSchedule(Number(id));
      toast.success('送迎スケジュールを削除しました');
      navigate('/sougei-schedule');
    } catch {
      toast.error('送迎スケジュールの削除に失敗しました');
    } finally {
      setDeleting(false);
    }
  };
...
export default SougeiScheduleEditPage;
